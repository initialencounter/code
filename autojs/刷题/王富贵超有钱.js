...
function final(){
    ...
    log('不同颜色的选项')
    log(da)
    var obje = className("android.view.View").depth("14").drawingOrder("0").indexInParent("2").findOnce().text()
    if(da.length == 1){//只有一个点颜色改变，选项一正确
        // tiku.put(wenti(),obj)//写入储存
        log('正确')
        log(obje)
    }else{//两个颜色改变
        var ob = obj()
...
function main(){
    ...
    if(tiku.contains(wt)){//找到题目
        ...
        var ram = Math.ceil(Math.random()*30);
        click(XY.centerX()+ram,XY.centerY()+ram)
